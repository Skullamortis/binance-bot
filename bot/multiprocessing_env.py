...
def train_multiprocessing(CustomEnv, agent, train_df, num_worker=4, training_batch_size=500, visualize=False, EPISODES=10000):
    ...
    while episode < EPISODES:
        ...
        for worker_id, parent_conn in enumerate(parent_conns):
            ...
            if reset:
                ...
                print(color1 + "Episode " + color_reset + "{:<5}Worker: {:<1} Net Worth: {:<7.2f} Average: {:<7.2f} Orders: {}".format(episode, worker_id, net_worth, average, episode_orders))
                if episode > len(total_average):
                    if best_average < average and net_worth > 1000:
                        best_average = average
                        print(color3 + "Saving model" + color_reset)
    ...
    for work in works:
        work.terminate()
        print(color2 + 'TERMINATED:' + color_reset, work)
        work.join()
    ...
    while episode < test_episodes:
        ...
        for worker_id, parent_conn in enumerate(parent_conns):
            ...
            if reset:
                ...
                if net_worth < initial_balance: no_profit_episodes += 1 
                print(color1 + "Episode " + color_reset + "{:<5}Worker: {:<1} Net Worth: {:<7.2f} Average_net_worth: {:<7.2f} Orders: {}".format(episode, worker_id, net_worth, average_net_worth/episode, episode_orders))
